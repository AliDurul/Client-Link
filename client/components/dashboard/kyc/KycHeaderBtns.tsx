'use client'
import { selectType, setType, setKyc } from '@/lib/features/kyc/kycSlice';
...
const KycHeaderBtns = () => {
    ...
    const value = useAppSelector(selectType)
    ...
    return (
        <>
            ...
            <div>
                <button type="button" className={`btn btn-outline-primary p-2 ${value === 'list' && 'bg-primary text-white'}`} onClick={() => dispatch(setType('list'))}>
            ...
            <div>
                <button type="button" className={`btn btn-outline-primary p-2 ${value === 'grid' && 'bg-primary text-white'}`} onClick={() => dispatch(setType('grid'))}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5">
                        <path
                            opacity="0.5"
