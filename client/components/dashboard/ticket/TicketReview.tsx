...
import { flagOp, priorityOp, statusOp } from './TicketConstants';
import { delData } from '@/lib/features/shared/actionUtils';
import { useRouter } from 'next/navigation';
import toast from 'react-hot-toast';
import { formatDate } from '@/lib/utility/functions';
import TicketUserSideProfile from './TicketUserSideProfile';
import Link from 'next/link';

export default function TicketReview({ ticket }: { ticket: Ticket }) {
    ...
    // functions
    const [stateDelete, actionDelete, isPendingDelete] = useActionState(delData, null);
    const handleDelete = async (id: string) => {
        startTransition(() => {
            actionDelete({ url: 'tickets', id });
        });
    ...
    return (
        <div className={` panel p-0  ${isEscalation ? 'sm:col-span-2' : 'sm:col-span-3'} ${lightBgColor}`}>
            ...
            <div className=" mb-1 flex items-start justify-between  p-5 ">
                <h5 className="sm:text-2xl text-sm font-extrabold ">Ticket / <span className='text-sm'>{ticket?._id ?? ''}</span></h5>
                    ...
                    <div className="dropdown">
                        ...
                        >
                            <ul className="text-sm font-medium">
                                ...
                                <li>
                                    <button type="button" onClick={() => handleDelete(ticket._id)}>
                                ...
                                </li>
                                {
                                    ticket.status !== "escalated" && (
                                        <li>
                                            <button
                                                type="button"
                                            // onClick={() => dispatch(setEscalate(true))}
                                            >
                                                <svg className="ltr:mr-3 rtl:ml-3" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* Body */}
            <div className='p-5 pt-0 pb-3 relative' key={ticket._id}>
                <form action="">
                    <div className="">
                        <div className="flex justify-between">
                            <div className="flex w-max items-center">
                                <div className="ltr:ml-2 rtl:mr-2">
                                    ...
                                    <div className="text-sx text-white-dark">{formatDate(ticket?.createdAt, true)}</div>
                                </div>
                            </div>
                            <div
                                className='text-center cursor-pointer'
                                onClick={() => setSideProfileModal(true)}
                            >
                        ...
                        <div className='my-5 '>
                            <div className="flex">
                                ...
                                <textarea rows={2} className="form-textarea ltr:rounded-l-none bg-transparent focus:border-dark-dark-light" ></textarea>
                            </div>
            ...
            {sideProfileModal && (
                <TicketUserSideProfile
                    sideProfileModal={sideProfileModal}
                    setSideProfileModal={setSideProfileModal}
                />
            )}
        </div>
    )
