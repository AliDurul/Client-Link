...
const TaskSidebar = ({ isShowTaskMenu, setIsShowTaskMenu, tasks, addEditTask }: TaskSidebarProp) => {
    ...
    const handleStatusChange = (newTab: string) => {
        const newUrl = formUrlQuery({
            params: searchParams.toString(),
            key: 'status',
            value: newTab === 'all' ? null : newTab
        });
    ...
    const handlePriorityChange = (newPriority: string) => {
        ...
        const isSamePriority = currentPriority === newPriority;
        const newUrl = formUrlQuery({
            params: searchParams.toString(),
            key: 'priority',
            ...
            <div className="flex h-full flex-col pb-16">
                ...
                <PerfectScrollbar className="relative -mr-3.5 h-full grow pr-3.5">
                    <div className="space-y-1">
                        <button
                            type="button"
                            className={`flex h-10 w-full items-center justify-between rounded-md p-2 font-medium hover:bg-white-dark/10 hover:text-primary dark:hover:bg-[#181F32] dark:hover:text-primary ${status === 'all' ? 'bg-gray-100 text-primary dark:bg-[#181F32] dark:text-primary' : ''
                                }`}
                            onClick={() => {
                                tabChanged();
                        ...
                        >
                            <div className="flex items-center">
                                ...
                                <div className="ltr:ml-3 rtl:mr-3">All</div>
                            </div>
                            <div className="whitespace-nowrap rounded-md bg-primary-light py-0.5 px-2 font-semibold dark:bg-[#060818]">
                                {tasks && tasks.result.filter((d) => d.status !== 'Cancelled').length}
                            </div>
                        </button>

                        <button
                            type="button"
                            className={`flex h-10 w-full items-center justify-between rounded-md p-2 font-medium hover:bg-white-dark/10 hover:text-primary dark:hover:bg-[#181F32] dark:hover:text-primary ${status === 'High' && 'bg-gray-100 text-primary dark:bg-[#181F32] dark:text-primary'
                                }`}
                            onClick={() => {
                                tabChanged();
                                handleStatusChange('High');
                            }}
                        >
                            <div className="flex items-center">
                            ...
                            <div className="whitespace-nowrap rounded-md bg-primary-light py-0.5 px-2 font-semibold dark:bg-[#060818]">
                                {tasks && tasks.result.filter((d) => d.priority === 'High').length}
                            </div>
                        </button>

                        <button
                            type="button"
                            className={`flex h-10 w-full items-center justify-between rounded-md p-2 font-medium hover:bg-white-dark/10 hover:text-primary dark:hover:bg-[#181F32] dark:hover:text-primary ${status === 'In Progress' && 'bg-gray-100 text-primary dark:bg-[#181F32] dark:text-primary'
                                }`}
                            onClick={() => {
                                tabChanged();
                        ...
                        >
                            ...
                            <div className="whitespace-nowrap rounded-md bg-primary-light py-0.5 px-2 font-semibold dark:bg-[#060818]">
                                {tasks && tasks.result.filter((d) => d.status === 'In Progress').length}
                            </div>
                        </button>

                        <button
                            type="button"
                            className={`flex h-10 w-full items-center justify-between rounded-md p-2 font-medium hover:bg-white-dark/10 hover:text-primary dark:hover:bg-[#181F32] dark:hover:text-primary ${status === 'Pending' && 'bg-gray-100 text-primary dark:bg-[#181F32] dark:text-primary'
                                }`}
                            onClick={() => {
                                tabChanged();
                        ...
                        >
                            ...
                            <div className="whitespace-nowrap rounded-md bg-primary-light py-0.5 px-2 font-semibold dark:bg-[#060818]">
                                {tasks && tasks.result.filter((d) => d.status === 'Pending').length}
                            </div>
                        </button>

                        <button
                            type="button"
                            className={`flex h-10 w-full items-center justify-between rounded-md p-2 font-medium hover:bg-white-dark/10 hover:text-primary dark:hover:bg-[#181F32] dark:hover:text-primary ${status === 'Completed' && 'bg-gray-100 text-primary dark:bg-[#181F32] dark:text-primary'
                                }`}
                            onClick={() => {
                                tabChanged();
                        ...
                        >
                            ...
                            <div className="whitespace-nowrap rounded-md bg-primary-light py-0.5 px-2 font-semibold dark:bg-[#060818]">
                                {tasks && tasks?.result.filter((d) => d.status === 'Completed').length}
                            </div>
                        </button>

                        <button
                            type="button"
                            className={`flex h-10 w-full items-center justify-between rounded-md p-2 font-medium hover:bg-white-dark/10 hover:text-primary dark:hover:bg-[#181F32] dark:hover:text-primary ${status === 'Cancelled' && 'bg-gray-100 text-primary dark:bg-[#181F32] dark:text-primary'
                                }`}
                            onClick={() => {
                                tabChanged();
                        ...
                        <button
                            type="button"
                            className={`flex h-10 w-full items-center rounded-md p-1 font-medium text-warning duration-300 hover:bg-white-dark/10 ltr:hover:pl-3 rtl:hover:pr-3 dark:hover:bg-[#181F32] ${priority === 'Low' && 'bg-gray-100 ltr:pl-3 rtl:pr-3 dark:bg-[#181F32]'
                                }`}
                            onClick={() => {
                                tabChanged();
                        ...
                        <button
                            type="button"
                            className={`flex h-10 w-full items-center rounded-md p-1 font-medium text-success duration-300 hover:bg-white-dark/10 ltr:hover:pl-3 rtl:hover:pr-3 dark:hover:bg-[#181F32] ${priority === 'Medium' && 'bg-gray-100 ltr:pl-3 rtl:pr-3 dark:bg-[#181F32]'
                                }`}
                            onClick={() => {
                                tabChanged();
                        ...
                        <button
                            type="button"
                            className={`flex h-10 w-full items-center rounded-md p-1 font-medium text-danger duration-300 hover:bg-white-dark/10 ltr:hover:pl-3 rtl:hover:pr-3 dark:hover:bg-[#181F32] ${priority === 'High' && 'bg-gray-100 ltr:pl-3 rtl:pr-3 dark:bg-[#181F32]'
                                }`}
                            onClick={() => {
                                tabChanged();
