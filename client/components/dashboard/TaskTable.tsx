...
import { selectAddTaskModal, setAddTaskModal, setTask, updateTasks } from '@/lib/features/task/taskSlice';
...
const TaskTable = ({ tasks }: { tasks: Task[] }) => {
    ...
    const addEditTask = (task: any) => {
        dispatch(setAddTaskModal(true));
        dispatch(setTask(task))
    }

    const taskCompleted = async (task: any = null) => {
        let itemIndex = tasks.findIndex((d: any) => d._id === task._id);
        if (itemIndex !== -1) {
    ...
    return (
        <>
                ...
                <tbody>
                    {
                        tasks.length > 0 ? tasks.map((task: Task, index: number) => {
                            return (
                                <tr className={`group cursor-pointer ${task.status === 'Completed' ? 'bg-white-light/30 dark:bg-[#1a2941] ' : ''}`} key={index}>
                                    <td className="w-1">
                                        <input
                                            type="checkbox"
                                            id={`chk-${task._id}`}
                                    ...
                                    <td className="w-1">
                                        <div className="flex w-max items-center justify-between">
                                            <div className="flex-shrink-0 ltr:mr-2.5 rtl:ml-2.5">
                                                {task.assigned_agent?.profile_pic && (
                                                    <Tippy content={`${task.assigned_agent.first_name} ${task.assigned_agent.last_name}`} theme="" >
                                                        <div>
                                                            <Image width={32} height={32} src={task.assigned_agent.profile_pic} className="h-8 w-8 rounded-full object-cover" alt="avatar" />
                                                        </div>
                                                    </Tippy>
            ...
            {addTaskModal && <TaskAddEditModal />}
            {viewTaskModal && <SelectedTaskModal setViewTaskModal={setViewTaskModal} viewTaskModal={viewTaskModal} selectedTask={selectedTask} />}
        </>
    )
}
